...
func TestMediaURLRejectsPrivateAddresses(t *testing.T) {
	for _, value := range []string{"http://127.0.0.1/a", "http://10.0.0.1/a", "http://169.254.169.254/latest", "http://[::1]/a", "file:///tmp/a"} {
...
func TestPublicIPClassification(t *testing.T) {
	...
	if isPublicIP(net.ParseIP("192.168.1.1")) || isPublicIP(net.ParseIP("2001:db8::1")) {
