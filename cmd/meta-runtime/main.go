...
func run(args []string) int {
	...
	runtimeServer, err := server.New(server.Config{Token: os.Getenv("META_RUNTIME_TOKEN"), Capabilities: []string{"e2ee.text", "facebook.marketplace", "facebook.notifications", "facebook.posts", "facebook.profile", "facebook.search", "facebook.social", "messenger.controls", "messenger.notes", "messenger.polls", "messenger.search", "messenger.send", "messenger.threads", "runtime.info", "session.events", "session.health", "session.lifecycle"}})
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
